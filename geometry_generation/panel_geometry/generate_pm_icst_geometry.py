...
"""
    ...
    def build_geometry(self):
        ...
        full_list_fuselage_panel += [[x_f3[i], y_f3[i]] for i in range(1, len(x_f3))]
        full_list_fuselage_panel += [[x_f4[i], y_f4[i]] for i in range(1, len(x_f4) - 10)]
        x_f_panel = np.linspace(x_f4[-11], x_f7[-1], 200)
        y_f_panel = [y_f4[-11] - y_f4[-11] / (x_f7[-1] - x_f4[-11]) * (x_f_panel[i] - x_f4[-11]) for i in
                     range(0, len(x_f_panel))]
        full_list_fuselage_panel += [[x_f_panel[i], y_f_panel[i]] for i in range(1, len(x_f_panel))]
        ...
        return full_list_fuselage, full_list_nacelle_top, full_nacelle_bottom_reverse, full_list_fuselage_panel, \
               l_fuse_tot, f_slr, tc_max, tc_max_x, i_nac, c_nac, np.rad2deg(teta_f_aft), Astar_A2, x_thr, x_12, x_13, \
               a_18, a_13, a_12
