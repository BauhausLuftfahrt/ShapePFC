...
"""
    ...
    def __init__(self, x_ff: float, r_cent_f: float, l_cent_f: float, rmax: float, f_xmax: float, lnac: float,
    ...
    def build_geometry(self):
        ...
        l_fuse_tot = self.x_ff + l_cone_f + l_ff_stage + l_noz
        l_f_preint = l_fuse_tot - self.l_cent_f - self.lnac - l_cone_f
        ...
        if self.x_ff / l_fuse_tot > 1.0 or self.f_xmax > 1.0 or self.f_r12 > 1.0 or self.f_lint > 1.0 or \
        ...
        elif any([full_list_nacelle_bottom[i][1] > r_cent_f for i in range(0, len(full_list_nacelle_bottom))]) or \
                (any([full_list_nacelle_top[i][1] > r_cent_f for i in range(0, len(full_list_nacelle_top))]) and
                 any(abs([full_list_nacelle_top[j][0] for j in
                          [i for i in range(len(full_list_nacelle_top)) if full_list_nacelle_top[i][1] > r_cent_f]] - (
                                 l_max + l_cent_f + l_f_preint)) > 1e-4)):
            raise Warning("Resulting nacelle geometry not valid. Y > r_cent_f.")
        ...
        return full_list_fuselage, full_list_nacelle_top, full_nacelle_bottom_reverse, rotor_inlet, rotor_outlet, \
               ...
               Astar_A2, x_thr, x_121_f, x_132_n, rotor_le_coeffs, stator_le_coeffs, h_duct_in, h_duct_out, a_12, a_13, a_18, x_nac_max_tot
